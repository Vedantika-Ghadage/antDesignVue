<template>
<div>
  <a-row justify='center'>
  <a-card hoverable style="width: 70%;align: center ">
    <template slot="actions" class="ant-card-actions">
      <a-icon key="edit" type="edit" />
    </template>
    <a-card-meta title="Profile" description="Account setting">
      <a-avatar
        slot="avatar"
        src="data:image/png;base64,"
      />
    </a-card-meta>
  </a-card>
  </a-row>
  <a-card title="Notification" hoverable style="width: 70%;marginTop: 16px ">
    <p style="fontSize: 14px;color: rgba(0, 0, 0, 0.85); marginBottom: 16px;marginTop: 16px;fontWeight: 500">
      Comments
    </p>

    <a-card title="Comments on your posts">
    <div slot="extra">
      <a-switch default-checked @change="onChange" />
   </div>
    </a-card>

    <a-card title="Comments that mention you">
    <div slot="extra">
      <a-switch default-checked @change="onChange" />
   </div>
    </a-card>
    <a-card title="Private comments on work" :style="{ marginTop: '16px' }">
    <div slot="extra">
      <a-switch default-checked @change="onChange" />
   </div>
    </a-card>
    <p style="fontSize: 14px;color: rgba(0, 0, 0, 0.85); marginBottom: 16px;marginTop: 16px;fontWeight: 500">
      Classes you teach
    </p>

    <a-card title="Late submissions of student work">
    <div slot="extra">
      <a-switch default-checked @change="onChange" />
   </div>
    </a-card>

    <a-card title="Resubmissions of student work">
    <div slot="extra">
      <a-switch default-checked @change="onChange" />
   </div>
    </a-card>

    <a-card title="Invitations to co-teach classes">
    <div slot="extra">
      <a-switch default-checked @change="onChange" />
   </div>
    </a-card>
    <a-card title="Scheduled post published or failed" :style="{ marginTop: '16px' }">
    <div slot="extra">
      <a-switch default-checked @change="onChange" />
   </div>
    </a-card>
  </a-card>
  </div>
</template>

<script>
export default {
  data () {
    return {}
  },
  methods: {
    onChange (checked) {
      console.log(`a-switch to ${checked}`)
    }
  }
}
</script>
